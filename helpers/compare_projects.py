...
def gather_pathways(categ_names: Iterable, tsv_dict: dict, n_dict: dict) -> pd.DataFrame:
    """Build pathway summary stats table for multiple projects.

    Args:
        categ_names: Ordered iterable of descriptive names for project.
        tsv_dict: maps categ_name -> path for all_pways tsv file.
        n_dict: Project name -> number of cases in project. Note: should
            also include cases without any variants.
    Returns:
        DataFrame: 1 row per pathway, columns are per-project statistics.
    """
    df_dict = {}
    ...
    df['gene_coverage'] = df['gene_coverage'].applymap(_parse_cov_struct)
    ...
    new_index = df.index.set_levels(short_names, level='pathway_name')
    df.index = new_index
    return df


def identify_enrichment(ref_name: str, compared_name: str, df: pd.DataFrame,
                        other_compared: Iterable = None, excel_out: bool = False,
                        ) -> pd.DataFrame:
    """Build enrichment comparison table for enrichment relative to a reference project.

    Args:
        ref_name: Reference project name. Enrichment is assessed relative to
            this project.
        compared_name: Name of project to be assessed relative to reference project.
        df: pathway stats table, output from gather_pathways.
        other_compared (iterable): (optional) names of other projects to include
            in table, adding columns with proportion test P values vs reference.
    ...
    if excel_out:
        xlsx_path = f'enrichment_{ref_name}_gt_{compared_name}.xlsx'
        out.to_excel(xlsx_path)
        _logger.info("Enrichment comparison table written to %s", xlsx_path)

    return out.copy()


def get_styled_comparison(comp: pd.DataFrame, caption: str) -> Styler:
    """Style enrichment comparison table.

    Args:
        comp: comparison table generated via identify_enrichment.
        caption: caption text for output.
...
def _parse_cov_struct(cov_struct):
